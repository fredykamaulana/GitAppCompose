...
fun observeSetFavourite(context: Context, result: Result<Long>, setFavouriteAction: () -> Unit) {
    when(result){
        is Result.Success -> {
            if(result.data == -1L) showToast(context, context.getString(R.string.favourite_save_conflict_message))
            else showToast(context, context.getString(R.string.favourite_save_success_message))
...
fun observeDeleteFavourite(context: Context, result: Result<Int>, deletedAction: () -> Unit) {
    when(result){
        is Result.Success -> {
            ...
            showToast(context, context.getString(R.string.favourite_delete_success_message))
        }
        is Result.Error -> { showToast(context, result.errorMessage.orEmpty()) }
        else -> {}
    }
